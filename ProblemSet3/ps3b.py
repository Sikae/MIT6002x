...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    pylab.title("Growth of Virus Population without drug resistance and patient without drugs")
    pylab.xlabel("Timesteps")
    pylab.ylabel("Virus Population")
    pylab.legend("Population Growth")
    pylab.show()
    #print population

#simulationWithoutDrug(50, 1000, 0.5, 0.05, 100)
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantTo(self, drug):
        ...
        if drug in self.resistances and self.resistances[drug]:
            return True
        return False
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for d in activeDrugs:
            if not self.resistances[d]:
                raise NoChildException()
        if random.random() > self.maxBirthProb * (1 - popDensity):
            raise NoChildException()
        offspring_r = self.resistances.copy()
        for r in self.resistances:
            if random.random() <= self.mutProb:
                offspring_r[r] = not offspring_r[r]
        return ResistantVirus(self.maxBirthProb, self.clearProb, offspring_r, self.mutProb)
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for v in virusesCopy:
            try:
                offspring = v.reproduce(popDensity, self.prescriptions)
            except NoChildException:
                continue
            self.viruses.append(offspring)
        return self.getTotalPop()
